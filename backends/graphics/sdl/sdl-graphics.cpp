...
Common::Keymap *SdlGraphicsManager::getKeymap() {
	...
	if (hasFeature(OSystem::kFeatureCRTEmulation)) {
		act = new Action("CRTE", _("Toggle CRT emulation"));
		act->addDefaultInputMapping("C+A+c");
		act->setCustomBackendActionEvent(kActionToggleCRTEmulation);
		keymap->addAction(act);
	}
